...
SCRIPTNAME<-"formatdata-OmegaPlus.R"
JOBID<-ARGS[1]
...
##Input $3: Directory containing OmegaPlus output files (per chromosome).
##Output: Formatted data to be used for plotting (.csv).

##Usage: 
##Rscript --vanilla formatdata-OmegaPlus.R <JOB ID> <OUTPUT LOCATION> <INPUT DIRECTORY>
...
OUTPUTFILEPREFIX<-system(paste0("echo ",INPUTDIRNAME,"| sed 's/-job[0-9].*$//'"),
                         intern=TRUE)
OUTPUTFILENAME<-paste0(OUTPUTFILEPREFIX,".OmegaPlusdata-job",JOBID,".csv")
...
FILES<-FILES[grep("OmegaPlus_Report.*$",FILES)]
...
for(C in CHRSEQ){
  ...
  if(length(FILECHRC)==1){
    ...
    TMPDATA<-read.table(FILECHRC,sep="\t",header=FALSE)
    colnames(TMPDATA)<-c("Location","Likelihood")
