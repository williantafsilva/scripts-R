...
SCRIPTNAME<-"formatdata-SweepFinder2.R"
JOBID<-ARGS[1]
...
##Input $3: Directory containing SweepFinder2 output files (per chromosome).
##Output: Formatted data to be used for plotting (.csv).

##Usage: 
##Rscript --vanilla formatdata-SweepFinder2.R <JOB ID> <OUTPUT LOCATION> <INPUT DIRECTORY>
...
OUTPUTFILEPREFIX<-system(paste0("echo ",INPUTDIRNAME,"| sed 's/-job[0-9].*$//'"),
                         intern=TRUE)
OUTPUTFILENAME<-paste0(OUTPUTFILEPREFIX,".SF2data-job",JOBID,".csv")
...
for(C in CHRSEQ){
  ...
  if(length(FILECHRC)==1){
    ...
    colnames(TMPDATA)<-c("Location","Likelihood","Alpha")
