...
SCRIPTNAME<-"geneannotations-biomart-gal7b.R"
JOBID<-ARGS[1]
...
#Input $3: Comma-separated file with Chromosome, Start and End columns.
#Output: File with annotations (*.csv).

#Usage: 
#Rscript --vanilla template-Rscript.R <JOB ID> <OUTPUT LOCATION> <CHROMOSOME> <START POSITION> <END POSITIONS>
...
OUTPUTFILEPREFIX<-system(paste0("echo ",INPUTFILENAME,"| sed 's/-job[0-9].*$//'"),
                         intern=TRUE)
OUTPUTFILENAME<-paste0(INPUTFILENAME,".biomartannot-job",JOBID,".txt")
...
#Load input data.
INPUTDATA<-read.csv(INPUTFILE,sep=',',header=TRUE)
